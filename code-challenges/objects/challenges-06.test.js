...
const hasChildrenValues = (arr, character) => {
  let potentialCharacter;
  arr.forEach(characterObj => {
    if (characterObj.name === character) {
      potentialCharacter = characterObj;
    }
  })
  return potentialCharacter && potentialCharacter.children.length > 0;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(characterObj => {
    count++;
    count += characterObj.children.length;
    if (characterObj.spouse) {
      count++;
    }
  })
  return count;
